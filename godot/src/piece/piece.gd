...
func move(dest: Vector2):
	var tw = create_tween()
	tw.tween_property(self, "global_position", dest, 1.0).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN_OUT)
	tw.finished.connect(func(): move_done.emit())

func matched():
	var tw = create_tween()
	tw.tween_property(self, "scale", Vector2(0, 0), 0.5).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN_OUT)
